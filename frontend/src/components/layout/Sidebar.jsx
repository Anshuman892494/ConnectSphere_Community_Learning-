...
import { Globe, X, Camera, CreditCard } from 'lucide-react';
  ...
  const navLinks = (
    <>
      ...
      <NavLink
        ...
        className={() =>
          `flex items-center gap-1 py-2 pl-2 pr-1 transition-colors ${
            location.pathname === '/settings' && new URLSearchParams(location.search).get('tab') === 'subscription'
              ? 'font-bold text-gray-900 bg-[#F1F2F3] border-r-[3px] border-[#F48024]'
              : 'hover:text-gray-900 hover:bg-gray-100/50'
          }`
        }
      >
        ...
        <span className="ml-1">{t('subscriptionPlan')}</span>
      </NavLink>
      ...
      <div className="mt-4 flex flex-col pl-2 pr-1">
        <div className="flex items-center justify-between text-[11px] text-gray-500 font-semibold tracking-wider mb-2 uppercase">
          ...
          <span 
            onClick={() => addToast('Collectives directory is coming soon!', 'info')}
        ...
        <div 
          onClick={() => addToast('Collectives directory is coming soon!', 'info')}
          className="text-xs text-gray-600 px-2 py-1 flex items-center gap-2 hover:bg-gray-100 cursor-pointer rounded-sm"
        >
      ...
      <div className="mt-4 flex flex-col pl-2 pr-1">
        <div className="flex items-center justify-between text-[11px] text-gray-500 font-semibold tracking-wider mb-2 uppercase">
          <span>Teams</span>
          <span 
            onClick={() => addToast('ConnectSphere Teams subscription is coming soon! Collaborate privately with your organization.', 'info')}
            className="text-gray-400 hover:text-blue-500 cursor-pointer"
          >
            What's this?
          </span>
        ...
        <NavLink 
          to="/settings?tab=subscription"
          onClick={handleLinkClick}
          className="px-2 text-xs text-gray-500 hover:text-gray-900 hover:bg-gray-100 cursor-pointer py-1.5 rounded-sm flex items-center gap-2"
        >
